...
/// of the module.
ExecutionEngine *PolyJIT::createJIT(Module *M,
                                std::string *ErrorStr,
...
class JitPool {
  SmallPtrSet<PolyJIT*, 1> JITs;  // Optimize for process containing just 1 JIT.
  mutable sys::Mutex Lock;
public:
  void Add(PolyJIT *jit) {
    MutexGuard guard(Lock);
    JITs.insert(jit);
  }
  void Remove(PolyJIT *jit) {
    MutexGuard guard(Lock);
    JITs.erase(jit);
  }
  void *getPointerToNamedFunction(const char *Name) const {
    ...
    for (SmallPtrSet<PolyJIT*, 1>::const_iterator Jit = JITs.begin(),
           end = JITs.end();
         Jit != end; ++Jit) {
...
bool PolyJIT::removeModule(Module *M) {
...
///
GenericValue PolyJIT::runFunction(Function *F,
...
void PolyJIT::RegisterJITEventListener(JITEventListener *L) {
  if (L == NULL)
    return;
  MutexGuard locked(lock);
  EventListeners.push_back(L);
}
void PolyJIT::UnregisterJITEventListener(JITEventListener *L) {
  if (L == NULL)
    return;
  MutexGuard locked(lock);
...
void PolyJIT::NotifyFunctionEmitted(
    const Function &F,
...
///
void PolyJIT::runJITOnFunction(Function *F, MachineCodeInfo *MCI) {
  MutexGuard locked(lock);
...
void PolyJIT::runJITOnFunctionUnlocked(Function *F, const MutexGuard &locked) {
...
void PolyJIT::jitTheFunction(Function *F, const MutexGuard &locked) {
...
///
void *PolyJIT::getPointerToFunction(Function *F) {
...
void PolyJIT::addPointerToBasicBlock(const BasicBlock *BB, void *Addr) {
  MutexGuard locked(lock);
...
void PolyJIT::clearPointerToBasicBlock(const BasicBlock *BB) {
  MutexGuard locked(lock);
  getBasicBlockAddressMap(locked).erase(BB);
}

void *PolyJIT::getPointerToBasicBlock(BasicBlock *BB) {
...
void *PolyJIT::getPointerToNamedFunction(const std::string &Name,
                                     bool AbortOnFailure){
...
/// Emitter.
void *PolyJIT::getOrEmitGlobalVariable(const GlobalVariable *GV) {
  MutexGuard locked(lock);
...
/// just like PolyJIT::getPointerToFunction().
///
void *PolyJIT::recompileAndRelinkFunction(Function *F) {
...
/// on the target.
///
char* PolyJIT::getMemoryForGV(const GlobalVariable* GV) {
  char *Ptr;
...
void PolyJIT::addPendingFunction(Function *F) {
  MutexGuard locked(lock);
