//===-- PolyJITEmitter.cpp - Write machine code to executable memory ----------===//
//
...
namespace {
  ...
  public:
    JITResolverState(PolyJIT *jit) : FunctionToLazyStubMap(this),
  ...
  public:
    explicit JITResolver(PolyJIT &jit, PolyJITEmitter &je)
  ...
  /// PolyJITEmitter - The JIT implementation of the MachineCodeEmitter, which is
  /// used to output functions to memory for execution.
  class PolyJITEmitter : public JITCodeEmitter {
    JITMemoryManager *MemMgr;
  
    // FIXME: linker goes boom without this
    ...
    struct EmittedFunctionConfig : public ValueMapConfig<const Function*> {
      typedef PolyJITEmitter *ExtraData;
      static void onDelete(PolyJITEmitter *, const Function*);
      static void onRAUW(PolyJITEmitter *, const Function*, const Function*);
    };
  ...
  public:
    PolyJITEmitter(PolyJIT &jit, JITMemoryManager *JMM, TargetMachine &TM)
...
// PolyJITEmitter code.
//
void *PolyJITEmitter::getPointerToGlobal(GlobalValue *V, void *Reference,
                                     bool MayNeedFarStub) {
...
void *PolyJITEmitter::getPointerToGVIndirectSym(GlobalValue *V, void *Reference) {
...
void PolyJITEmitter::processDebugLoc(DebugLoc DL, bool BeforePrintingInsn) {
...
void PolyJITEmitter::startFunction(MachineFunction &F) {
...
bool PolyJITEmitter::finishFunction(MachineFunction &F) {
...
void PolyJITEmitter::retryWithMoreMemory(MachineFunction &F) {
...
void PolyJITEmitter::deallocateMemForFunction(const Function *F) {
...
void *PolyJITEmitter::allocateSpace(uintptr_t Size, unsigned Alignment) {
  if (BufferBegin)
...
void *PolyJITEmitter::allocateGlobal(uintptr_t Size, unsigned Alignment) {
  // Delegate this call through the memory manager.
  return MemMgr->allocateGlobal(Size, Alignment);
}

void PolyJITEmitter::emitConstantPool(MachineConstantPool *MCP) {
...
void PolyJITEmitter::initJumpTableInfo(MachineJumpTableInfo *MJTI) {
...
void PolyJITEmitter::emitJumpTableInfo(MachineJumpTableInfo *MJTI) {
  if (TheJIT->getJITInfo().hasCustomJumpTables())
    return;
...
void PolyJITEmitter::startGVStub(const GlobalValue* GV,
...
void PolyJITEmitter::startGVStub(void *Buffer, unsigned StubSize) {
  SavedBufferBegin = BufferBegin;
  SavedBufferEnd = BufferEnd;
  SavedCurBufferPtr = CurBufferPtr;
...
void PolyJITEmitter::finishGVStub() {
...
void *PolyJITEmitter::allocIndirectGV(const GlobalValue *GV,
                                  const uint8_t *Buffer, size_t Size,
                                  unsigned Alignment) {
...
// method.
//
uintptr_t PolyJITEmitter::getConstantPoolEntryAddress(unsigned ConstantNum) const {
...
//
uintptr_t PolyJITEmitter::getJumpTableEntryAddress(unsigned Index) const {
...
void PolyJITEmitter::EmittedFunctionConfig::onDelete(
  PolyJITEmitter *Emitter, const Function *F) {
  Emitter->deallocateMemForFunction(F);
}
void PolyJITEmitter::EmittedFunctionConfig::onRAUW(
  PolyJITEmitter *, const Function*, const Function*) {
  llvm_unreachable("The JIT doesn't know how to handle a"
                   " RAUW on a value it has emitted.");
}
...
JITCodeEmitter *PolyJIT::createEmitter(PolyJIT &jit, JITMemoryManager *JMM,
                                       TargetMachine &tm) {
  return new PolyJITEmitter(jit, JMM, tm);
}
...
//
void *PolyJIT::getPointerToFunctionOrStub(Function *F) {
  // If we have already code generated the function, just return the address.
  if (void *Addr = getPointerToGlobalIfAvailable(F))
    return Addr;

  // Get a stub if the target supports it.
  PolyJITEmitter *JE = static_cast<PolyJITEmitter*>(getCodeEmitter());
  return JE->getJITResolver().getLazyFunctionStub(F);
}

void PolyJIT::updateFunctionStub(Function *F) {
...
///
void PolyJIT::freeMachineCodeForFunction(Function *F) {
  // Delete translation for this from the ExecutionEngine, so it will get
  // retranslated next time it is used.
  updateGlobalMapping(F, 0);

  // Free the actual memory for the function body and related stuff.
  static_cast<PolyJITEmitter*>(JCE)->deallocateMemForFunction(F);
}
