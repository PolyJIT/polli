...
// CHECK-NEXT:   store i8* bitcast ([10240 x i32]* @StructA.0 to i8*), i8** %8
// CHECK-NEXT:   %9 = bitcast [3 x i8*]* %params to i8*
// CHECK-NEXT:   %10 = call i1 @pjit_main(i8* getelementptr inbounds ([{{[0-9]+}} x i8], [{{[0-9]+}} x i8]* @test{{[0-9a-zA-Z\.\_]+}}.pjit.scop.prototype, i32 0, i32 0), i32 3, i8* %9)
