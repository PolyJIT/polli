; RUN: opt -S -load LLVMPolyJIT.so -polli-process-unprofitable -polly-use-runtime-alias-checks=false -polli-detect-scops -jitable -polly-detect-keep-going -analyze < %s 2>&1 | FileCheck %s
