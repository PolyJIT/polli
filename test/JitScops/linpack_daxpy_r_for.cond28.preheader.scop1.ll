; RUN: opt -S -load LLVMPolly.so -load LLVMPolyJIT.so -polli-process-unprofitable -polly-use-runtime-alias-checks=false -polli-detect-scops -polli -analyze < %s 2>&1 | FileCheck %s
