...
; RUN: opt -load LLVMPolyJIT.so -O3 -jitable -polli -polli-detect-scops-scops-keep-going -no-recompilation -polli-analyze -disable-output -stats < %s 2>&1 | FileCheck %s
