...
; RUN: opt -load LLVMPolyJIT.so -O3 -jitable -polly-process-unprofitable -polli -polly-only-scop-detection -polly-delinearize=false -polly-detect-keep-going -no-recompilation -polli-analyze -disable-output -stats < %s 2>&1 | FileCheck %s
