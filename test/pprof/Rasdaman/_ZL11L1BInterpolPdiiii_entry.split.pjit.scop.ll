...
; RUN: opt -load LLVMPolyJIT.so -O3 -jitable -polli-process-unprofitable -polly-process-unprofitable -polli  -no-recompilation -polli-analyze -disable-output -stats < %s 2>&1 | FileCheck %s
