...
; RUN: opt -load LLVMPolly.so -load LLVMPolyJIT.so -O3  -polli-process-unprofitable -polly-process-unprofitable -polli  -polli-no-recompilation -polli-analyze -disable-output -stats < %s 2>&1 | FileCheck %s
