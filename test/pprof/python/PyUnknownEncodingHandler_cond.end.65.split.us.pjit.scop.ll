; RUN: opt -load LLVMPolyJIT.so -O3 -jitable -polli -polly-only-scop-detection -polli-process-unprofitable  -no-recompilation -polli-analyze -disable-output -stats < %s 2>&1 | FileCheck %s
