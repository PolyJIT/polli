; RUN: opt -load LLVMPolly.so -load LLVMPolyJIT.so -O3  -polli -polli-process-unprofitable  -polli-no-recompilation -polli-analyze -disable-output -stats < %s 2>&1 | FileCheck %s
